...
void MenuHandler::handleEvent(AceButton *button, uint8_t eventType, uint8_t buttonState)
{
    ...
    case AceButton::kEventLongPressed:
        ESP_LOGI("MenuHandler", "Long press detected - toggling menu");
...
void MenuHandler::executeMenuItem()
{
    switch (menuPosition)
    {
    case 0: // Start Capturing
        collector.begin();
        menuActive = false;
        break;

    case 1: // Start Inferring
...
void MenuHandler::drawMenu()
{
    ...
    display.drawStr(0, 63, "Click: Next  DblClick: Select");
...
void MenuHandler::drawDefaultScreen()
{
    ...
    display.drawStr(0, 6, ("v" + String(VERSION)).c_str());
...
void MenuHandler::update()
{
    ...
    if (firstDraw || is_redraw || (millis() - lastUpdate > 1000)) {  // Update every second or when needed
